...
public struct Subscription: Sendable,Identifiable {
    ...
    public struct Included: Sendable {
        ...
        init?(included: [SubscriptionResponse.IncludedItem]?) {
            self.subscriptionLocalizations = included?.compactMap { (item: SubscriptionResponse.IncludedItem) -> SubscriptionLocalization? in
                if case let .subscriptionLocalization(value) = item { return .init(schema: value) }
                return nil
            }
            
            self.subscriptionPrices = included?.compactMap { (item: SubscriptionResponse.IncludedItem) -> SubscriptionPrice? in
                if case let .subscriptionPrice(value) = item { return .init(schema: value) }
                return nil
            }
            
            self.subscriptionGroup = included?.compactMap { (item: SubscriptionResponse.IncludedItem) -> SubscriptionGroup? in
                if case let .subscriptionGroup(value) = item { return .init(schema: value) }
                return nil
            }.first
            
            self.subscriptionAppStoreReviewScreenshot = included?.compactMap { (item: SubscriptionResponse.IncludedItem) -> ImageAsset? in
            ...
            self.promotedPurchase = included?.compactMap { (item: SubscriptionResponse.IncludedItem) -> PromotedPurchase? in
                if case let .promotedPurchase(value) = item { return .init(schema: value) }
                return nil
            }.first
            
            self.subscriptionOfferCodes = included?.compactMap { (item: SubscriptionResponse.IncludedItem) -> SubscriptionOfferCode? in
                if case let .subscriptionOfferCode(value) = item { return .init(schema: value) }
                return nil
            }
            
            self.subscriptionAvailability = included?.compactMap { (item: SubscriptionResponse.IncludedItem) -> Bool? in
                if case let .subscriptionAvailability(value) = item { return value.attributes?.isAvailableInNewTerritories }
                return nil
            }.first
            
            self.introductoryOffers = included?.compactMap { (item: SubscriptionResponse.IncludedItem) -> SubscriptionIntroductoryOffer? in
                if case let .subscriptionIntroductoryOffer(value) = item { return .init(schema: value) }
                return nil
            }
            
            self.promotionalOffers = included?.compactMap { (item: SubscriptionResponse.IncludedItem) -> SubscriptionPromotionalOffer? in
                if case let .subscriptionPromotionalOffer(value) = item { return .init(schema: value) }
                return nil
            }
            
            self.winBackOffers = included?.compactMap { (item: SubscriptionResponse.IncludedItem) -> WinBackOffer? in
                if case let .winBackOffer(value) = item { return .init(schema: value) }
                return nil
            }
            
            self.subscriptionImages = included?.compactMap { (item: SubscriptionResponse.IncludedItem) -> SubscriptionImage? in
