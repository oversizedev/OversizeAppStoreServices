...
public struct GameCenterLeaderboardVersionV2: Sendable, Identifiable {
    public let id: String
    public let version: Int?
    public let state: State?
    public let relationships: Relationships?

    public init?(schema: AppStoreAPI.GameCenterLeaderboardVersionV2) {
        id = schema.id
        version = schema.attributes?.version
        state = schema.attributes?.state.flatMap { State(rawValue: $0.rawValue) }
        relationships = .init(
            leaderboardId: schema.relationships?.leaderboard?.data?.id,
